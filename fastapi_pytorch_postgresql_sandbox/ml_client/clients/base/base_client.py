...
from fastapi_pytorch_postgresql_sandbox.ml_client._logging import _WithLogDetailsClient
from fastapi_pytorch_postgresql_sandbox.ml_client._utils import _to_safe_id, ignore_docs
...
class _BaseBaseClient(metaclass=_WithLogDetailsClient):
    ...
    def _url(self, path: Optional[str] = None) -> str:
        return f"{self.url}/{path}" if path is not None else self.url
