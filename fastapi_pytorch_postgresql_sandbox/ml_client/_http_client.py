...
from fastapi_pytorch_postgresql_sandbox.ml_client._errors import (
    ApifyApiError,
    InvalidResponseBodyError,
    _is_retryable_error,
)
from fastapi_pytorch_postgresql_sandbox.ml_client._logging import logger_name
from fastapi_pytorch_postgresql_sandbox.ml_client._types import JSONSerializable
...
class _BaseHTTPClient:
    ...
    def _maybe_parse_response(response: httpx.Response) -> Any:
        ...
        try:
            ...
            elif _is_content_type_xml(content_type) or _is_content_type_text(
                content_type,
            ):
    ...
    def _prepare_request_call(
        ...
        if json and data:
            raise ValueError(
                'Cannot pass both "json" and "data" parameters at the same time!',
            )
        ...
        *,
        ...
        def _make_request(stop_retrying: Callable, attempt: int) -> httpx.Response:
            ...
            try:
                ...
                logger.debug(
                    f"Request to {url} threw exception",
                    exc_info=e,
